...
function initPanel2() {
  ...
  setTimeout(() => { cu1.start(); cu2.start(); cu3.start(); }, 200);
}
...
function buildChoropleth() {
  ...
  fetch(GEOJSON_URL)
    ...
    .then(geojson => {
      ...
      /* Filtrar features sin nombre (polígono vacío de Malvinas, etc.)
         para que fitExtent abarque solo las provincias argentinas */
...
function buildTreemap() {
  ...
  /* Zoom via D3 transitions — scale from cell center */
  cell
    .on('mouseenter', function(event, d) {
      ...
      d3.select(this)
        ...
        .attr('transform',
          `translate(${d.x0 + cx},${d.y0 + cy}) scale(1.07) translate(${-cx},${-cy})`
        );
    })
  ...
  cell.each(function(d) {
    ...
    /* % — tamaño grande, centrado verticalmente */
    const pxPct = Math.min(Math.max(cw * 0.18, 14), big ? 28 : 20);
 
    /* Posición vertical: si hay subtexto, sube un poco; si no, centra */
    const pctY = (big || med) ? ch * 0.42 : ch / 2;
    ...
    /* Subtextos: nombre y cantidad — mismo tamaño, cerca del borde inferior */
    const subPx = Math.min(Math.max(cw * 0.075, 9), 12);
    const pad   = 7;  /* padding desde el borde inferior */
 
    if (big) {
      /* Nombre: una línea arriba de la cantidad */
      g.append('text')
        .attr('class', 'treemap-sub')
        .attr('x', cw / 2)
        .attr('y', ch - pad - subPx - 3)
        .attr('text-anchor', 'middle')
        .attr('dominant-baseline', 'auto')
        .attr('font-size', subPx + 'px')
        .text(d.data.name);
 
      /* Cantidad: pegada al borde inferior */
      g.append('text')
        .attr('class', 'treemap-sub')
        .attr('x', cw / 2)
        .attr('y', ch - pad)
        ...
        .attr('dominant-baseline', 'auto')
        .attr('font-size', subPx + 'px')
        .text(d.data.value + ' CCDs');
 
    } else if (med) {
      /* Solo nombre abreviado al pie */
      g.append('text')
        .attr('class', 'treemap-sub')
        .attr('x', cw / 2)
        .attr('y', ch - pad)
